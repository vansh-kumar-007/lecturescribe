...
class _CriticalBox(Flowable):
    """A purple-bordered box for critical blocks — height grows with content."""
    ...
    def _get_lines(self, width: float) -> list[str]:
        """Wrap text into lines that fit within the box width."""
        from reportlab.pdfbase.pdfmetrics import stringWidth
        available = width - 2 * self._padding
        words = self._text.split()
        lines, current = [], ""
        for word in words:
            test = (current + " " + word).strip()
            if stringWidth(test, self._font, self._font_size) <= available:
                current = test
            else:
                if current:
    ...
    def wrap(self, availWidth, availHeight):
        ...
        self._lines = self._get_lines(self._w)
        self._h = len(self._lines) * self._line_height + 2 * self._padding
    ...
    def draw(self):
        ...
        y = self._h - self._padding - self._line_height + 3
...
def _build_cover(notes: dict, styles: dict) -> list:
    ...
    topics = notes.get("topics_covered", [])
    read_time = notes.get("estimated_read_time_minutes", 0)
    ...
    elements = [
        ...
        Paragraph(f"Estimated read time: {read_time} min", styles["cover_date"]),
        Spacer(1, 6),
    ]

    if topics:
        topics_text = " · ".join(topics)
        elements.append(Paragraph(f"Topics: {topics_text}", styles["cover_footer"]))
        elements.append(Spacer(1, 4))

    elements += [
        Spacer(1, 60 * mm),
        HRFlowable(width="60%", thickness=0.5, color=C_RULE, spaceAfter=8),
    ...
    for i, block in enumerate(blocks):
        ...
        if btype == "heading":
            ...
            if png_path and os.path.exists(png_path):
                ...
                max_w = page_width * 0.85
                max_h = 180 * mm  # hard cap: never taller than 180mm

                scale_w = max_w / orig_w
                scale_h = max_h / orig_h
                scale = min(scale_w, scale_h)  # use whichever is more restrictive
